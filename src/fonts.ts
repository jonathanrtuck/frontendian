import { Font } from "@/types";

export const FONT_CHARCOAL: Font = {
  format: "truetype",
  id: "font-charcoal",
  title: "Charcoal",
  url: `${process.env.PUBLIC_URL}/fonts/Charcoal.ttf`,
};

export const FONT_DIGITAL_COURIER_10_PITCH: Font = {
  format: "opentype",
  id: "font-courier-10-pitch",
  title: "Courier10Pitch",
  url: `${process.env.PUBLIC_URL}/fonts/Courier10Pitch.otf`,
};

export const FONT_DIGITAL_DISMAY: Font = {
  format: "opentype",
  id: "font-digital-dismay",
  title: "DigitalDismay",
  url: `${process.env.PUBLIC_URL}/fonts/DigitalDismay.otf`,
};

export const FONT_MINE_SWEEPER: Font = {
  format: "opentype",
  id: "font-mine-sweeper",
  title: "MineSweeper",
  url: `${process.env.PUBLIC_URL}/fonts/MineSweeper.otf`,
};
